...
#define POOL_TAG 'TAGM'
      ...
      )
  {
	...
	//RtlInitUnicodeString(&str, L"YA SDELAL"); //MYSTR
      ...
      )
  {
      ...
      if (NT_SUCCESS(status)) {
		  ...
		  //RtlCopyMemory(p_DVCEXT->DataBuffer, str.Buffer, str.Length); //MYSTR
    ...
    )
{
	...
	PIO_STACK_LOCATION p_IO_STK;
	PDEVICE_EXTENSION p_DVCEXT = DeviceObject->DeviceExtension;
		
	UCHAR charbuff; //MYREADPORT
	charbuff = READ_PORT_UCHAR(p_DVCEXT->SourcePointer); //MYREADPORT
	...
	*(PUCHAR)DataBuf = charbuff; //MYREADPORT
    ...
    )
{
	...
	RtlCopyMemory(
		...
		WRITE_PORT_UCHAR(p_DVCEXT->SourcePointer, *(PUCHAR)DataBuf);//MYWRITEPORT
