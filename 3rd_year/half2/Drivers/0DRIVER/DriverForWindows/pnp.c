...
default:
	...
	return IoCallDriver(p_DVCEXT->NextDeviceObject, Irp);
	break;
}
	...
	StartDevice( IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	PCM_PARTIAL_RESOURCE_DESCRIPTOR JustResources;
	PCM_PARTIAL_RESOURCE_DESCRIPTOR TranslatedResources;
	...
	for (i = 0; i < count; i++)
	{
		JustResources = &(p_IO_STK->Parameters.StartDevice.AllocatedResources->List[0].PartialResourceList.PartialDescriptors[i]);
		TranslatedResources = &(p_IO_STK->Parameters.StartDevice.AllocatedResourcesTranslated->List[0].PartialResourceList.PartialDescriptors[i]);
		
		switch(JustResources->Type) 
		{ 
			case CmResourceTypePort: DbgPrint("LOOK HE FOUND ResourcePORT .................. \r\n"); break; 
			case CmResourceTypeMemory: DbgPrint("LOOK HE FOUND ResourceMEMORY .............. \r\n"); break; 
		} 
		
		switch(TranslatedResources->Type) 
		{ 
			case CmResourceTypePort: DbgPrint("LOOK HE FOUND ResourceTranslatedPORT ........ \r\n"); break; 
			case CmResourceTypeMemory: DbgPrint("LOOK HE FOUND ResourceTranslatedMEMORY .... \r\n"); break; 
		} 
	}
	
	p_DVCEXT->SourcePointer = JustResources->u.Port.Start.u.LowPart;
	p_DVCEXT->SourceSize = JustResources->u.Port.Length;
	DbgPrint("Size = %d \r\n", p_DVCEXT->SourceSize);
